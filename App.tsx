  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        {notification && (
          <div id="toast-notification" className="fixed bottom-4 right-4 bg-gray-900 text-white px-6 py-3 rounded-xl shadow-2xl z-[100]">
            {notification}
          </div>
        )}
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            {cart.length === 0 ? (
              <div className="text-center py-12 border-2 border-dashed rounded-2xl bg-white" id="empty-cart-msg">
                ...
                <button onClick={() => navigate('shop')} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-semibold">Start Shopping</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-4" id="cart-items-list">
                  {cart.map(item => (
                    <div key={item.id} className="bg-white p-4 border rounded-xl flex items-center space-x-4">
                      ...
                      <button onClick={() => removeFromCart(item.id)} className="text-gray-400 hover:text-red-600 p-2" id={`btn-remove-${item.id}`}><Trash2 className="h-5 w-5" /></button>
                    </div>
                  ))}
                </div>
                <div className="bg-white p-6 border rounded-xl h-fit space-y-6">
                  ...
                  <button id="btn-checkout" onClick={() => navigate('checkout')} className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition">Proceed to Checkout</button>
                </div>
              </div>
            )}
          </div>
        ...
        {view === 'checkout' && (
          <div className="max-w-3xl mx-auto space-y-8">
            ...
            <form id="checkout-form" onSubmit={(e) => { e.preventDefault(); setCart([]); navigate('success'); }} className="bg-white p-8 border rounded-2xl shadow-sm space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Full Name</label>
                  <input id="input-name" required type="text" placeholder="John Doe" className="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Email Address</label>
                  <input id="input-email" required type="email" placeholder="john@example.com" className="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
              </div>
              <button id="btn-place-order" type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition">Place Order Now</button>
            </form>
          </div>
        )}
        ...
        {view === 'success' && (
          <div className="max-w-md mx-auto text-center py-20 space-y-6" id="order-success-container">
            ...
            <button id="btn-return-home" onClick={() => navigate('shop')} className="px-8 py-4 bg-indigo-600 text-white rounded-xl font-bold">Back to Shop</button>
          </div>
        )}
        ...
        {view === 'login' && (
          <div className="max-w-md mx-auto pt-12">
            <div className="bg-white p-8 border rounded-2xl shadow-xl space-y-8">
              ...
              <form id="login-form" onSubmit={(e) => { e.preventDefault(); setUser("admin"); navigate('shop'); }} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Username</label>
                  <input id="login-username" required defaultValue="admin" type="text" className="w-full px-4 py-3 border rounded-xl outline-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Password</label>
                  <input id="login-password" required defaultValue="password123" type="password" className="w-full px-4 py-3 border rounded-xl outline-none" />
                </div>
                <button id="btn-login-submit" type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold">Log In</button>
              </form>
            </div>
          </div>
        )}
        ...
        {view === 'test-cases' && (
          <div className="space-y-8 max-w-5xl mx-auto">
            <h1 className="text-3xl font-bold border-b pb-4">Automation Test Cases</h1>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="test-cases-grid">
              {TEST_CASES.map(tc => (
                <div key={tc.id} className="bg-white border rounded-2xl p-6 shadow-sm" id={`test-case-${tc.id}`}>
                  <h3 className="font-bold text-indigo-700 mb-2">{tc.id}: {tc.title}</h3>
                  <p className="text-sm text-gray-600 mb-4">{tc.description}</p>
                  <div className="text-xs font-bold uppercase text-gray-400 mb-1">Expected Result</div>
                  <p className="text-xs text-gray-900 italic">{tc.expectedResult}</p>
                </div>
              ))}
            </div>
          </div>
